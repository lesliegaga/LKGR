...
class LKGR(torch.nn.Module):
    ...
    def _batch_score(self, user_index, item_index):
        ...
        print('ckeck_9', torch.cuda.memory_allocated(0), torch.cuda.memory_allocated(1))

        self.batch_size = item_index.shape[0]
        user_ngh = self._get_user_ngh(item_index)
        print('ckeck_10', torch.cuda.memory_allocated(0), torch.cuda.memory_allocated(1))
        entities, relations = self._get_entity_ngh(item_index)
        print('ckeck_11', torch.cuda.memory_allocated(0), torch.cuda.memory_allocated(1))
        all_item_embedding = self.entity_embedding(item_index)
        print('ckeck_12', torch.cuda.memory_allocated(0), torch.cuda.memory_allocated(1))

        new_all_item_embeddings = self._aggregate_for_item(all_item_embedding, user_ngh, entities, relations)

        print('ckeck_13', torch.cuda.memory_allocated(0), torch.cuda.memory_allocated(1))
        new_all_user_embedding = self.manifold.logmap0(new_all_user_embedding, self.c)
        print('ckeck_14', torch.cuda.memory_allocated(0), torch.cuda.memory_allocated(1))

        score = torch.mm(new_all_user_embedding, new_all_item_embeddings.t())
        print('ckeck_15', torch.cuda.memory_allocated(0), torch.cuda.memory_allocated(1))
        all_score = torch.sigmoid(score)
        print('ckeck_16', torch.cuda.memory_allocated(0), torch.cuda.memory_allocated(1))
        all_score = torch.clamp(all_score, min=self.min_norm, max=self.max_norm)
        print('ckeck_17', torch.cuda.memory_allocated(0), torch.cuda.memory_allocated(1))
